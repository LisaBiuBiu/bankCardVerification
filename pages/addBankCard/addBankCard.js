var bankList = require('../../common/bankList.js');
...
Page({
    ...
    getUserIdCardNumber: function (e) {
        ...
        let url = `https://ccdcapi.alipay.com/validateAndCacheCardInfo.json?_input_charset=utf-8&cardNo=${this.data.bankNumber}&cardBinCheck=true`

        COMMON.getData("GET", url, {
            config: {},
            mbcSuc: (res) => {
                if (res.data.validated) {
                    console.log(res.data)
                    let bankName = this.getBankName(res.data.bank);
                    this.setData({
                        cardType: bankName
                    })
                    this.getBankPic(res.data.bank);
                } else {
                    util.toast('卡号验证错误，请确认卡号是否正确')
                }
    ...
    getBankPic (bank) { // 得到银行的卡的图片
        let url =`https://apimg.alipay.com/combo.png?d=cashier&t=${bank}`;
        this.setData({
            bankUrl:url
        })
        // COMMON.getData("GET", url, {
